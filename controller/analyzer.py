...
def main(args):
    ...
    if multiple:
        if not args.resume:
            ...
            clean(args.output)
        if args.parallel:
            parallel_projects_analysis(refactor, args.input, args.output, args.max_workers,resume)
        else:
            if not os.path.exists(f"{args.output}"):
                os.makedirs(f"{args.output}")
            projects_analysis(refactor, args.input, args.output,resume)
    else:

        analyze_project(refactor, args.input, args.output)
